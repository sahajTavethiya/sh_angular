...
@Component({
  selector: 'app-ticket-closure-report',
  templateUrl: './ticket-closure-report.component.html',
  styleUrls: ['./ticket-closure-report.component.scss']
})

export class TicketClosureReportComponent implements OnInit {
  ...
  bindColumns() {
    this.cols = [
      ...
      { header: 'SR date', field: 'date' },
      { header: 'SR Status', field: 'srStatus' },
      { header: 'Customer Name', field: 'customerName' },
      { header: 'Mob. no.', field: 'mobileNo' },
  ...
  public export(json: any[]) {
    ...
    json.forEach(element => {
      let singalData = {
        ...
        'SR no.' : element.srNo,
        'SR date' : element.date != null ? this.service.Moment(element.date).format("MM-DD-YYYY"):'',
        'SR Status' : element.srStatus,
        'Customer Name' : element.customerName,
        'Mob. no.' : element.mobileNo,
    ...
    XLSX.utils.book_append_sheet(workBook, workSheet, 'Ticket closure'); // add the worksheet to the book
    XLSX.writeFile(workBook, 'Ticket closure Report '+Dateto+'.xlsx'); // initiate a file download in browser
  }
