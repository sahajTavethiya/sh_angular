...
export class CustomerMasterComponent implements OnInit {
  ...
  search() {
    ...
    if (this.searchForm.valid) {
      ...
      this.service.getCustomerListForGrid().subscribe((response: any) => {
        console.log('Customer Master Response --', response);
        ...
        if (response && response.data && response.data.listData.length > 0) {
          ...
          this.totalRecords = response.data.listData[0].RowCount; //response.data[0].RowCount;
  ...
  addOrder() {
    this.routeUrl.navigate(['/CustomerDetail']);
  }
  reset() {
