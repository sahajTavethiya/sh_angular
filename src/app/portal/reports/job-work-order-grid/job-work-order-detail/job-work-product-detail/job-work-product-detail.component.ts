...
import { SellService } from '../../../sell-master/sell.service';
import { JObWorkProductDetail } from 'src/app/library/core/models/report/jobWorkOrder/product-detail.model';
@Component({
  selector: 'app-job-work-product-detail',
  templateUrl: './job-work-product-detail.component.html',
  styleUrls: ['./job-work-product-detail.component.scss']
})
export class JObWorkProductDetailComponent implements OnInit {
  ...
  constructor(
    ...
    const userService = new JObWorkProductDetail();
  ...
  public get RequestDetail() {
    return this.requestForm.controls.JobWorkProductDetail as FormGroup;
  }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const assignedManpowerLists = this.requestForm.controls
          .JobWorkProductDetail as FormArray;
  ...
  addSparesConsumables(taskType: JObWorkProductDetail) {
    ...
    if (taskType) {
      const newTaskType = new JObWorkProductDetail(taskType);
      ...
      console.log(this.requestForm.controls.JObWorkProductDetail);

      console.log("This is a request fporm",this.requestForm)
      this.isInputVisible = false;
