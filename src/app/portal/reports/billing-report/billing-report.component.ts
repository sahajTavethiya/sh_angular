...
export class BillingReportComponent implements OnInit {
  ...
  bindColumns() {
    this.cols = [
      ...
      { header: 'BP Nos', field: 'customerId' },
      { header: 'CA No.', field: 'caNo' },
      { header: 'Customer Name', field: 'customerName' },
      { header: 'Mobile No.', field: 'mobileNo' },
      { header: 'Quantity', field: 'quantity' },
      { header: 'H No', field: 'houseNo' },
      { header: 'Floor', field: 'floor' },
      { header: 'Society Name', field: 'society' },
      { header: 'Area', field: 'area' },
      { header: 'Meter Nos', field: 'meterNo' },
      { header: 'JMR No.', field: 'jmrNo' },
      { header: 'NG Date', field: 'gasStartDate' },
      { header: 'WBS', field: 'wbs' },
      { header: 'STATUS', field: 'description' },
      { header: 'Upload Status', field: 'uploadStatus' },
  ...
  search() {
    ...
    if (this.searchForm.valid) {
      ...
      this.service.GetBillReport(this.searchData).subscribe((response: any) => {
  ...
  exportCSV() {
    ...
    this.service.GetBillReport(searchData).subscribe((response: any) => {
      if (response && response.data && response.data.result.length > 0) {
  ...
  public export(json: any[]) {
    ...
    json.forEach(element => {
      let singalData = {
        'SR No' : element.id,
        'BP Nos' : element.customerId,
        'CA No.' : element.caNo,
        'Customer Name' : element.customerName,
        'Mobile No.' : element.mobileNo,
        'Quantity' : element.quantity,
        'H No' : element.houseNo,
        'Floor' : element.floor,
        'Society Name' : element.society,
        'Area' : element.area,
        'Meter Nos' : element.meterNo,
        'JMR No.' : element.jmrNo,
        'NG Date' : element.gasStartDate != null ?this.service.Moment(element.gasStartDate).format("MM-DD-YYYY"):'' ,
        'WBS' : element.wbs,
    ...
    XLSX.utils.book_append_sheet(workBook, workSheet, 'Billing Report'); // add the worksheet to the book
    XLSX.writeFile(workBook, 'Billing Report '+Dateto+'.xlsx'); // initiate a file download in browser
  }
