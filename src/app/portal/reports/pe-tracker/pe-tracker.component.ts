...
@Component({
  selector: 'app-pe-tracker',
  templateUrl: './pe-tracker.component.html',
  styleUrls: ['./pe-tracker.component.scss']
})

export class PeTrackerComponent implements OnInit {
 ...
 //   const searchCriteria = new SearchSRCriteria();
  ...
  public export(json: any[]) {
    ...
    XLSX.utils.book_append_sheet(workBook, workSheet, 'PE Tracking Report'); // add the worksheet to the book
    XLSX.writeFile(workBook, 'PE Tracking Report '+Dateto+'.xlsx'); // initiate a file download in browser
  }
    ...
    if (this.getbulkSelect.length != 0) {
      ...
      this.service.DownloadJMRs(this.serviceIdsName).subscribe((data:any) => {
        if(data.status = "success"){
          ...
          link.download ="PE_TrackerReports";
          link.click();
