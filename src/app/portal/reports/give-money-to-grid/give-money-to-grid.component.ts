...
import { MoneyDetailService } from '../get-money-from-grid/money-detail.service';

@Component({
  selector: 'app-give-money-to-grid',
  templateUrl: './give-money-to-grid.component.html',
  styleUrls: ['./give-money-to-grid.component.scss']
})
export class GiveMoneyToGridComponent  implements OnInit {
  constructor(
    readonly service: MoneyDetailService,
  ...
  //    this.showAddButton = response.data.result.find((obj: any) => obj.resourceId == environment.ResourceMasterIds.WorkerReport)?.canInsert;
  //  })
  ...
  bindColumns() {
    this.cols = [
      { header: 'Id', field: 'Id' },
    { header: 'Give Money To', field: 'giveMoneyTo' },
  ...
  search() {
    ...
    if (this.searchForm.valid) {
      ...
      this.service
      .GetListOfRemainingPaymentToPay(this.searchData)
      .subscribe((response: any) => {
        if (response && response.data && response.data.listData.length > 0) {
          this.Material_Details = response.data.listData;
