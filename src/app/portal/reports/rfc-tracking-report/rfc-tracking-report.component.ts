...
@Component({
  selector: 'app-rfc-tracking-report',
  templateUrl: './rfc-tracking-report.component.html',
  styleUrls: ['./rfc-tracking-report.component.scss']
})

export class RFCTrackingReportComponent implements OnInit {
  ...
  bindColumns() {
    this.cols = [
      ...
      { header: 'Regulator No', field: 'regulatorno' },
      { header: 'RFC Date', field: 'gasStartDate' },
      { header: 'House No.', field: 'houseNo' },
      { header: 'Society', field: 'society' },
      { header: 'Area', field: 'area' },
      { header: 'Completed By', field: 'completedBy' },
      { header: 'Verified By', field: '' },
      { header: 'Remarks', field: 'remarks' },
      { header: 'RFC Reading', field: 'rfcReading' },
      { header: 'JMR Reading', field: 'jmrReading' },
      { header: 'Burner Details', field: 'burnerDetails' },
      { header: 'JMR No', field: 'jmrNo' },
      { header: 'Gas Start Date', field: '' },
      { header: 'Riser GI Pipe 1/2”', field: 'riser_GI_Pipe_1_2_' },
      { header: 'Riser GI Pipe 3/4”', field: 'riser_GI_Pipe_3_4_' },
      { header: 'Riser IV 1/2”', field: 'riser_IV_1_2_' },
      { header: 'Riser IV 3/4”', field: 'riser_IV_3_4_' },
      { header: 'Tapping GI Pipe 1/2”', field: 'tapping_GI_Pipe_1_2_' },
      { header: 'Tapping GI Pipe 3/4”', field: 'tapping_GI_Pipe_3_4_' },
      { header: 'Tapping IV 1/2”', field: 'tapping_IV_1_2_' },
      { header: 'Copper Tube', field: 'copperTube' },
    ];
  }
  sotingData(name: any) {
    if (name != "action") {
  ...
  search() {
    ...
    if (this.searchForm.valid) {
      ...
      if(data.employeeName != null && data.employeeName != ''){
        var ss = 'Employee Name:'+this.employeeNames.filter((x:any) => x.keyName == data.employeeName)[0].displayText;
        this.displayArr.push(ss);
      }
  ...
  public export(json: any[]) {
    ...
    json.forEach(element => {
      let singalData = {
       'SR Number': element.id,
       'BP no': element.customerID,
       'Customer Name': element.customerName,
       'Mobile no.': element.mobileNo,
       'Meter No': element.meterNo,
       'Regulator No': element.regulatorno,
       'RFC Date': element.gasStartDate != null ? this.service.Moment(element.gasStartDate).format("MM-DD-YYYY") : '',
       'House No.': element.houseNo,
       'Society': element.society,
       'Area': element.area,
       'Completed By': element.completedBy,
       'Verified By': '',
       'Remarks': element.remarks,
       'RFC Reading': element.rfcReading,
       'JMR Reading': element.jmrReading,
       'Burner Details': element.burnerDetails,
       'JMR No': element.jmrNo,
       'Gas Start Date': '',
       'Riser GI Pipe 1/2”': element.riser_GI_Pipe_1_2_,
       'Riser GI Pipe 3/4”': element.riser_GI_Pipe_3_4_,
       'Riser IV 1/2”': element.riser_IV_1_2_,
       'Riser IV 3/4”': element.riser_IV_3_4_,
       'Tapping GI Pipe 1/2”': element.tapping_GI_Pipe_1_2_,
       'Tapping GI Pipe 3/4”': element.tapping_GI_Pipe_3_4_,
       'Tapping IV 1/2”': element.tapping_IV_1_2_,
       'Copper Tube': element.copperTube,
      }
    ...
    XLSX.utils.book_append_sheet(workBook, workSheet, 'RFC Tracking Report'); // add the worksheet to the book
    XLSX.writeFile(workBook, 'RFC Tracking Report '+Dateto+'.xlsx'); // initiate a file download in browser
  }
    ...
    if (this.getbulkSelect.length != 0) {
      ...
      this.service.DownloadJMRs(this.serviceIdsName).subscribe((data:any) => {
        if(data.status = "success"){
          ...
          link.download ="RFC_TrackerReports";
          link.click();
        }else{
