...
export class EmployeeWorkReportComponent implements OnInit {
  constructor(
    readonly service: EmployeeWorkReportService,
  ...
  cols = [
    { header: ' Id', field: 'Id' },
    { header: 'Employee Name', field: 'issuedByName' },
    { header: 'MonthYear', field: 'MonthYear' },
    { header: 'Total Work', field: 'TotalWork' },
  ];
  ...
  //    this.showAddButton = response.data.result.find((obj: any) => obj.resourceId == environment.ResourceMasterIds.EmployeeWorkerReport)?.canInsert;
  //  })
  ...
  search() {
    ...
    //   this.service
    //     .getWorkerAccountReport(this.searchData)
    ...
    // }
        ...
        .subscribe((response: any) => {
          if (response && response?.data && response.data.data.length > 0) {
            this.Material_Details = response.data.data;
            this.paginateData[0] = 0;
