  ...
  search(){
    ...
    if (this.searchForm.valid) {
      ...
      this.searchData = {
        ...
        orderBy: this.sotingname == '' ? 'UM.Id  ' : this.sotingname + ' ' + this.OrderBy,
     ...
     console.log(this.displayArr);
      ...
      this.service.getUserReport(this.searchData).subscribe((response: any) => {
        if (response && response.data) {
          this.Material_Details = response.data.userList;
          this.paginateData[0] = 0;
         this.totalRecords = response.data.rowData[0].RowCount;
        }
        else {
